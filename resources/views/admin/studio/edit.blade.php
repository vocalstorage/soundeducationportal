...
@section('content')
    <div class="row">
        <div class="col s12">
            <div style="float:left;"><h1>{{trans('modules/studio.function.edit')}}</h1></div>
        </div>
    </div>
    <div class="row">
        <div class="col s12">
            <form action="{{route('admin-studio-update',$studio->id)}}" method="post">
                ...
                <div class="input-field">
                    <input id="name" type="text" class="validate {{ $errors->has('name') ? ' invalid' : '' }}"
                           value="{{old('name') ? old('name') : $studio->name}}" name="name">
                ...
                <div class="row">
                    <div class="input-field col s5">
                        <input id="place" type="text" class="validate {{ $errors->has('place') ? ' invalid' : '' }}"
                               value="{{old('place') ? old('place') : $studio->place}}" name="place">
                    ...
                    <div class="input-field col s5">
                        <input id="street" type="text" class="validate {{ $errors->has('street') ? ' invalid' : '' }}"
                               value="{{old('street') ? old('street') : $studio->street}}" name="street">
                    ...
                    <div class="input-field col s2">
                        <input id="number" type="text" class="validate {{ $errors->has('number') ? ' invalid' : '' }}"
                               value="{{old('number') ? old('number') : $studio->number}}" name="number">
                ...
                <div class="input-field">
                    <input id="postal_code" type="text"
                           ...
                           value="{{old('postal_code') ? old('postal_code') : $studio->postal_code}}"
                           name="postal_code">
                ...
                <div class="input-field">
                    <select id="teacher_id" class="validate {{ $errors->has('teacher_id') ? ' invalid' : '' }}"
                        ...
                        @foreach($teachers as $teacher)
                            <option @if($studio->teacher->id === $teacher->id) selected
                ...
                <div class="file-field input-field col s10">
                    ...
                    <div class="file-path-wrapper">
                        <input id="thumbnail" class="validate {{ $errors->has('filepath') ? ' invalid' : '' }}"
                               type="text"
                               name="filepath" value="{{old('filepath') ? old('filepath') : $studio->filepath->path}}">
